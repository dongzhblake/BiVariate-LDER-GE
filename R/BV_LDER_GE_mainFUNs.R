...
#' @param n.ld The sample size of the LD reference panel
...
runBV_LDER_GE <- function(assoc_gwis, assoc_gwas, n.gwis, n.gwas, n.overlap=NULL, path, LD.insample=T,  n.ld,method='lder',size_num=200){
  ...
  if(method=='lder'){
    ...
    return(list(lder = res, lder.BV_test = bitest))
  }else if(method=='ldsc'){
    res <- bivariate_ldsc(bivariate_stats,N1,N2,NS=NS,size_num=size_num)
    bitest=test_bivariate_GE(res,"BVN")
    return(list(ldsc = res, ldsc.BV_test = bitest))
  }else if(method=='both'){
    ...
    return(list(lder=res1,ldsc=res2, lder.BV_test = bitest1, ldsc.BV_test = bitest2))
  }
}
