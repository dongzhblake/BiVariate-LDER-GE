...
runBV_LDER_GE_adj <- function(assoc_gwis_Y, assoc_gwas_Y, assoc_gwas_E, n.gwis_Y, n.gwas_Y, n.gwas_E, path, LD.insample=T,  n.ld,method='lder',size_num=200,R2=0){
  ...
  N3 = n.gwas_E
  if(method=='lder'){
    res <- bivariate_lder_correct(trivariate_stats,N1,N2,N3,size_num=200,R2,print_GIE=T)
    bitest=test_bivariate_GE(res,"BVN")
    return(list(lder = res, BV_test_lder = bitest))
  }else if(method=='ldsc'){
    res <- bivariate_ldsc_correct(trivariate_stats,N1,N2,N3,size_num=200,R2,print_GIE=T)
    bitest=test_bivariate_GE(res,"BVN")
    return(list(ldsc = res, BV_test_ldsc = bitest))
  }else if(method=='both'){
    res1 <- bivariate_lder_correct(trivariate_stats,N1,N2,N3,size_num=200,R2,print_GIE=T)
    res2 <- bivariate_ldsc_correct(trivariate_stats,N1,N2,N3,size_num=200,R2,print_GIE=T)
